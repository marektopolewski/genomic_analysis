...
int main (int argc, char * argv[])
{
    ...
    if (argc < 5) {
        ...
        std::cerr << "    <LNG> length of a SAM sequence\n";
    ...
    auto sequenceLength = std::stoi(argv[3]);
    auto outputPath = argv[4];
    ...
    ReferenceHandler refHandler(referencePath, sequenceLength);
    if (!refHandler.valid()) {
    ...
    VariantHandler variantHandler(outputPath, sequenceLength);
